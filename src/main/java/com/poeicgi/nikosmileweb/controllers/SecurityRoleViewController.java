...
public class SecurityRoleViewController extends AntoineViewBaseController<SecurityRole>{
	...
	protected final static String PATH_INDEX = PATH_BASE + "/" + index;

	protected final static String PATH_USERS = PATH_BASE + "/" + associationMultiShow;
	protected final static String PATH_USERSLINKS = PATH_BASE + "/"
			+ associationMultiEdit;
	protected final static String PATH_USERSLINKS_REDIRECT = REDIRECT + "/"
			+ ROUTE_REDIRECT + "/" + index;

	protected final static String PROJECT_ID = "{securityRoleId}";
	protected final static String ROUTE_INDEX = index;

	protected final static String ROUTE_USERS = PROJECT_ID + "/" + users;
	protected final static String ROUTE_USERSLINKS = PROJECT_ID + "/"
			+ usersLinks;

	public SecurityRoleViewController() {
		super(SecurityRole.class, BASE_URL);
		this.basePage = index;
		this.createRedirect = REDIRECT + this.basePath + "/" + ROUTE_INDEX;
		this.deleteRedirect = REDIRECT + this.basePath + "/"  + ROUTE_INDEX;
		this.updateRedirect = REDIRECT + this.basePath + "/"  + ROUTE_INDEX;
		this.showRedirect = REDIRECT + this.basePath + "/"  + ROUTE_INDEX;
		this.listRedirect = REDIRECT + this.basePath + "/"  + ROUTE_INDEX;
	}
